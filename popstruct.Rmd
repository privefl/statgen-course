...
- Linkage Disequilibrium (LD) structure, when there are two many correlated variants (e.g. within long-range LD regions) and not enough population structure (see [this vignette](https://privefl.github.io/bigsnpr/articles/how-to-PCA.html)),
...
:::: {.infobox .exo}
Use `big_randomSVD()` to perform a SVD/PCA on `G`. Do not forget to use some scaling (at least some centering). Look at PC scores and PC loadings with `plot()`. What do you see for PC4?
::::
...
**I recommend using only the first 16 PCs provided by the UK Biobank [@prive2020efficient].**
...
To achieve this, we can project this data onto the PCA space of many known population groups defined in @prive2021using (using the UK Biobank and some individuals from the 1000 Genomes).
...
proj_mat <- as.matrix(projection[matched$`_NUM_ID_`[ind], -(1:5)])
...
all_centers <- crossprod(as.matrix(all_freq[matched$`_NUM_ID_`[ind], -(1:5)]), proj_mat)
