...
WORDS_TO_IGNORE <- c("scalability", "PLR", "substack", "suboptimal", "PRS", "kb" , "Rightarrow", "MAF", "PRSice", "SBayesR", "SBayesRC", "allelic", "chr", "HapMap", "pos", "se")
...
If you have individual-level data (i.e. genotypes and phenotypes), you can basically use any supervised learning (machine learning) method to build PGS.
...
- within both males and females, PGS achieved 65.5% of correlation ($r^2$ of 42.9%) between genetically predicted and true height
...
($\hat\beta$ are the GWAS effects sizes)
::::
...
In GWAS, it is commonly accepted to use a p-value threshold of $5 \times 10^{-8}$ when reporting significant findings, which corresponds to correcting for one million independent tests. 
Yet for prediction purposes, including less significant variants can substantially improve predictive performance of C+T.
Therefore, the p-value threshold must be chosen carefully in C+T. A stringent threshold risks excluding informative variants, while a lenient threshold may introduce noise by including non-informative variants.
...
- threshold on squared correlation of clumping (e.g. $r_c^2 > 0.2$) 
...
- an implementation to efficiently compute thousands of C+T scores corresponding to different sets of hyper-parameters (to explore more)
...
- C+T is known to be suboptimal but works okay for traits with low polygenicity (the proportion of causal variants) 

- lassosum is very good at getting very sparse solutions (lots of variants are not used); C+T can be good for that as well

- LDpred2 is the best in my very biased opinion &#128521;; there is an "auto" option to directly infer hyper-parameters of the methods from the data, and many developments have been made regarding improving robustness

- SBayesR is very good with in-sample LD (i.e. from the GWAS data), otherwise it lacks robustness

- SBayesRC uses functional annotations to help prioritize causal variants, which can help improve prediction, especially when applying the PGS to other populations

- PRS-CS uses a lot of regularization to ensure robustness, which can be at the expense of lower predictive ability

```{r, echo=FALSE, out.width="75%", fig.cap="Comparison of PGS methods from the LDpred2 paper."}
...
- Predictive power $r^2$ is bounded by the heritability $h^2$ captured by the set of variants used (called SNP-heritability).
...
Basically, predictive performance drops with genetic distance from the training population [@prive2021high; @ding2023polygenic]:
...
One possible explanation: different tagging, because correlation between genetic variants are different across populations [@wang2020theoretical]:
...
- Integrate functional annotations and multi-ancestry data to prioritize causal variants

## Exercise with LDpred2 and lassosum2
...
You can still use the reference panel to do some quality control by comparing allele frequencies, which is useful to detect allelic errors (inversion of alleles and therefore effect sizes) and the other issues detected before:
```{r}
