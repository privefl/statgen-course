...
:::: {.infobox .info}
You can now use LDpred2-auto for inference of e.g. the SNP-heritability and polygenicity (@prive2022inferring + [tutorial](https://privefl.github.io/bigsnpr/articles/LDpred2.html#inference-with-ldpred2-auto)).
::::
...
This can lead to achieving much better predictive performance compared to some standard use of C+T.
...
- lassosum is very good at getting very sparse solutions (lots of variants are not used in the PGS); C+T can be good for that as well (when a stringent p-value threshold is used)

- LDpred2 is the best in my very biased opinion &#128521;; there is an "auto" option to directly infer hyper-parameters of the methods from the data, and many developments have been made regarding improving its robustness

- SBayesR is very good with in-sample LD (i.e. from the GWAS data), otherwise it lacks robustness

- SBayesRC uses functional annotations to help prioritize causal variants, which can help improve prediction, especially when applying the resulting PGS to other populations

- PRS-CS uses a lot of regularization internally, which makes it very robust, but sometimes at the expense of lower predictive ability; it is also slower than other methods

```{r, echo=FALSE, out.width="75%", fig.cap="Comparison of PGS methods from the LDpred2 paper. Since, developments have been made to ensure even better predictive performance of LDpred2 models."}
...
- $r^2$ should increase with GWAS sample size $N$ (of course)
